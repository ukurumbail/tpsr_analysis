...
@click.option('--only_ir',default=False,help='Just collate IR data, not TPSR data.')
def main(analyze,inert,ir_data_exists,only_ir):
	...
	if os.path.exists(dir_path):
		...
		#collate T, MS, potentially IR data
		print("\nGenerating dataset...")
		df = generate_csv(dir_path,IR_data_exists=ir_data_exists,only_IR=only_ir)

		if only_ir: #just an IR dataset
			df.to_csv(dir_path+"/DRIFTS.csv",index=False)
			print("\nSuccessully generated DRIFTS.csv.")

		else:

			if ir_data_exists: #TPSR dataset with IR
				df.to_csv(dir_path+"/TPSR_dataset_with_IR.csv",index=False)
				print("\nSuccessully generated TPSR_dataset_with_IR.csv.")
			else: #TPSR dataset without IR
